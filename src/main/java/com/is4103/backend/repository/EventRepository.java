...
public interface EventRepository extends JpaRepository<Event, Long>, JpaSpecificationExecutor<Event> {
        ...
        Page<Event> findByNameContainingAndIsHiddenAndEventStartDateGreaterThan(String name, boolean isHidden,
                        LocalDateTime eventStartDate, Pageable pageable);

        Page<Event> findByIsHiddenAndEventStartDateGreaterThan(boolean isHidden, LocalDateTime eventStartDate,
                        Pageable pageable);

        // @Query(value = "SELECT DISTINCT categories FROM event_categories",
        // nativeQuery = true)
        // List<String> getDistinctEventCategories();

        // @Query(value = "SELECT Count(id) as applicationCount, event_eid as eventId
        // FROM seller_application sa GROUP BY event_eid", nativeQuery = true)
        ...
        // @Query(value = "SELECT Count(category), e.category from event e GROUP BY
        // e.category, event_status, e.is_hidden,e.event_organiser_id HAVING
        // e.event_status = 'CREATED' and e.is_hidden = 0 and e.event_organiser_id =
        // ?1",nativeQuery = true)
        ...
        // nativeQuery = true)
        // List<String> getDistinctEventCategories();
        @Query(value = "SELECT DISTINCT e.category FROM Event e")
        List<String> getDistinctEventCategories();
}